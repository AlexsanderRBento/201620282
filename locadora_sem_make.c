...
void excluir_filme()
{
	...
	if(str_somente_numeros(str_id_filme) == 1)
	{
		...
		{
			char nome_filme[1000];

			FILE *arq_temp = fopen("temp_filmes.txt", "a+b");
			if(arq_temp == NULL)
			{
			...
			if(remove("filmes.txt") != 0)
				printf("\nErro ao deletar o arquivo \"filmes.txt\"\n");
			else
			{
				// renomeia o arquivo
				int r = rename("temp_filmes.txt", "filmes.txt");
				if(r != 0)
				{
